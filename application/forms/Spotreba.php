...
class Application_Form_Spotreba extends ZendX_JQuery_Form
{

    public function init()
    {
        $this->setName('spotreba');

        $id = new Zend_Form_Element_Hidden('ts_spotreby_id');
        $id->addFilter('Int');
        ...
        $datum_spotreby = new ZendX_JQuery_Form_Element_DatePicker("datum_spotreby_d",
            "12.12.2014", array(), array());

        $datum_spotreby->setValue(Zend_Date::now()->toString('YYYY-MM-dd'));

        $datum_spotreby->setJQueryParam('dateFormat', 'yy-mm-dd')
            ->setRequired(true)
            ->setLabel("Dátum spotreby")
            ->addFilter('StripTags')
